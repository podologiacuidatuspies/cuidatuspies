# Documento de Requisitos: Sitio Web Cuida Tus Pies - Podóloga Laura Núñez Morales

## 1. Nombre del Sitio Web
Cuida Tus Pies

## 2. Descripción del Sitio Web
Sitio web profesional para servicios de podología que ofrece atención integral especializada, combinando evaluación experta, diagnóstico preciso y tratamientos efectivos. El sitio permite a los usuarios conocer los servicios, planes de atención, productos podológicos y agendar consultas fácilmente.

## 3. Funcionalidades Principales

### 3.1 Sección Hero (Portada)
- Título principal H1: 'Podología Profesional: Salud, Bienestar y Confianza en Cada Paso'
- Subtítulo con llamada a la acción: '👣 Cuidamos tus pies como se merecen. ✨ Evaluación, tratamiento y prevención especializada. Agenda tu consulta y recupera la comodidad'\n- Botón principal: '📱 Agendar por WhatsApp' (enlace directo a WhatsApp)
\n### 3.2 Sección de Propuesta de Valor\n- Título: 'Tu Bienestar Comienza por la Base'
- Texto descriptivo sobre el servicio integral que ofrece Laura Núñez Morales, enfocado en salud y confort de los pies con trato cercano y profesional

### 3.3 Servicios Especializados
- Título: 'Atención Personalizada para Cada Necesidad'
- Presentación en tarjetas o iconos con los siguientes servicios:
  - Tratamientos Comunes: Callosidades, durezas, hongos (micosis), uñas encarnadas y engrosadas
  - Podología Clínica y Diabética: Enfoque especializado con máxima prioridad en seguridad y derivación oportuna
  - Atención a Domicilio: Para personas con movilidad reducida o preferencia por el hogar\n  - Convenios Grupales: Tarifas preferenciales para empresas, comunidades o grupos familiares

### 3.4 Planes de Atención
- Título: 'Elige el Plan que Tus Pies Necesitan'
- Tabla comparativa con tres planes:
\n**Plan Podología Básica (🥿)**
- Duración: 45 minutos
- Inversión: $25.000
- Ideal para: Mantenimiento y limpieza general
- Incluye: Corte y limado de uñas, retiro de durezas, limpieza de surcos, hidratación y masaje podal

**Plan Podología Clínica / Diabética (🏥)**\n- Duración: 60 minutos
- Inversión: $27.500
- Ideal para: Micosis leve, durezas, resequedad o cuidado preventivo diabético
- Incluye: Corte, limado y fresado, eliminación de durezas plantares, hidratación, masaje y derivación profesional si se requiere

**Plan Podología Intensiva (⚡)**
- Duración: 60 minutos
- Inversión: $30.000
- Ideal para: Casos avanzados con uñas engrosadas, involutas o micosis avanzada
- Incluye: Eliminación profunda de hiperqueratosis, tratamiento de helomas ('ojos de gallo'), fresado intensivo, hidratación profunda y masaje relajante

### 3.5 Beneficios y Accesibilidad
- Título: 'Por Qué Elegirnos'
- Lista de beneficios:
  - 🏠 Atención Domiciliaria
  - 👥 Convenios Grupales
  - 📱 Agenda Simple vía WhatsApp
  - 👩‍⚕️ Profesionalismo con técnicas seguras\n  - 🔗 Red de Derivación a especialistas
  - 🧴 Productos Podológicos: Contamos con diversos productos para hongos, mal olor e higiene

### 3.6 Catálogo de Productos Podológicos
- Título: 'Complementa Tu Tratamiento'
- Productos con imágenes y botón 'Consultar':
  - Esmalte de Terbinafina 3% (10 ml): Tratamiento intensivo para hongos - $7.990
  - Talco con Óxido de Zinc (30 g): Absorbe humedad y protege el pie - $3.990
  - Jabón Antimicótico (80 g): Previene la aparición de hongos - $3.590

### 3.7 Llamado a la Acción Final\n- Título: '¿Listo para dar el primer paso hacia el bienestar?'
- Texto: 'Contáctanos hoy mismo para una evaluación personalizada. Resolvemos tus dudas y te ayudamos a elegir el mejor plan para tu salud podal'
- Botón destacado: '💬 Agendar Consulta por WhatsApp'

## 4. Configuración de Supabase

### 4.1 Archivo de Configuración
Crear el archivo src/lib/supabase.js con el siguiente contenido:

```javascript
import { createClient } from '@supabase/supabase-js'

const supabaseUrl = import.meta.env.VITE_SUPABASE_URL\nconst supabaseAnonKey = import.meta.env.VITE_SUPABASE_ANON_KEY

export const supabase = createClient(
  supabaseUrl,
  supabaseAnonKey
)\n```

### 4.2 Variables de Entorno
Configurar las siguientes variables de entorno en el archivo .env:
- VITE_SUPABASE_URL: URL del proyecto Supabase
- VITE_SUPABASE_ANON_KEY: Clave anónima del proyecto Supabase
\n## 5. Estilo de Diseño
\n- **Paleta de colores**: Tonos pastel suaves de azul claro, verde agua y beige, transmitiendo calma, limpieza y bienestar profesional
- **Tipografía**: Fuente sans-serif limpia y legible para títulos y textos, asegurando fácil lectura en todos los dispositivos
- **Imágenes**: Fotografías de alta calidad mostrando pies cuidados, kit profesional y la podóloga en acción, evitando imágenes genéricas de stock
- **Diseño responsive**: Adaptación perfecta a móviles, tablets y desktop con navegación fluida
- **Estructura visual**: Uso de tarjetas, tablas comparativas y listas con iconos para presentar información de forma escaneable y organizada
- **Botones CTA**: Diseño destacado en color contrastante con iconos, ubicados estratégicamente en múltiples secciones\n- **Optimización**: Carga rápida mediante compresión de imágenes y código optimizado